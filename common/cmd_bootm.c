...
#if defined(CONFIG_FLASH_LAYOUT_TYPE2) || defined(CONFIG_FLASH_LAYOUT_TYPE3)
#ifdef CONFIG_DUAL_IMAGE
static int dual_image_sel (int index);
#endif /* CONFIG_DUAL_IMAGE */
static int boardid_get (void);
static int boardid_set (int id);
#endif /* CONFIG_FLASH_LAYOUT_TYPE2 */
...
#ifdef CONFIG_DUAL_IMAGE
#define PARTITION_ADDR_0 CONFIG_SYS_FLASH_BASE + KERNEL_START
#define PARTITION_ADDR_1 CONFIG_SYS_FLASH_BASE + KERNEL2_START
...
int do_boota (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
    ...
    for (arg=1; arg <argc; ++arg) {
        index = simple_strtoul(argv[arg], NULL, 10);
        if (dual_image_sel (index) != 0) rcode = 1;
    }
    return rcode;
}
...
static int dual_image_sel (int index)
{
    ...
    if (-1 == index)
    {
        ...
        if ( partition == IMG_PART1 )
        {
            ...
            memcpy(&ih, (const void *) PARTITION_ADDR_1, sizeof(ih));
            totalSize = ih.ih_size + sizeof(ih);
            if (totalSize <= KERNEL_SIZE)
            {
                memcpy((void *) RUNTIME_SRC_ADDR, (const void *) PARTITION_ADDR_1, totalSize);
                rc = run->cmd(run, 0, 2, argu);
            }
            else
            {
                rc = 1;
        ...
        else
        {
            ...
            memcpy(&ih, (const void *) PARTITION_ADDR_0, sizeof(ih));
            totalSize = ih.ih_size + sizeof(ih);
            if (totalSize <= KERNEL_SIZE)
            {
                memcpy((void *) RUNTIME_SRC_ADDR, (const void *) PARTITION_ADDR_0, totalSize);
                rc = run->cmd(run, 0, 2, argu);
            }
            else
            {
...
int do_boardid (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
    int arg;
    int id = -1;
    int rcode = 0;
    ...
    //for (arg=1; arg <argc; ++arg) {
    //    id = simple_strtoul(argv[arg], NULL, 10);
    //    if (boardid_set (id) != 0) rcode = 1;
    //}
    //return rcode;
}
...
static int boardid_get (void)
{
    ...
    idStr = getsys(SYSINFO_VAR_BOARDID);

    printf("Board Model ID: %s\n", idStr);
    return 1;
}
...
static int boardid_set (int id)
{
    ...
    sprintf(valStr, "%d", id);
    setsys(SYSINFO_VAR_BOARDID, valStr);
    savesys();
