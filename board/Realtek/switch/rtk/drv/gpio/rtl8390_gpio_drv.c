...
void rtl8390_intGpio_DirSet(uint32 pin, drv_gpio_direction_t dir)
{
    uint32 val;
    uint32 offset;
...
void rtl8390_intGpio_pindata_set(uint32 pin, uint32 bitData)
{
    uint32 val;
    uint32 offset;
            
    offset = gpio_info[pin].bitoffset;
    rtl8390_intGpio_DirSet(pin, GPIO_DIR_OUT);
...
void rtl8390_intGpio_pindata_get(uint32 pin, uint32 *pBitData)
{
    uint32 val;
    uint32 offset;
    
    offset = gpio_info[pin].bitoffset;
    rtl8390_intGpio_DirSet(pin, GPIO_DIR_IN);
    val = REG32(GPIO_PABC_DATA);
